...
void prefix_bit_ops(sockaddr_storage *addr, unsigned int prefix_length, bool set) {
  if (addr->ss_family == AF_INET) {
    ...
    as_v4->sin_addr.s_addr = htonl(set
      ? (ntohl(as_v4->sin_addr.s_addr) | prefix)
      : (ntohl(as_v4->sin_addr.s_addr) & ~prefix));
  }
    ...
    if (prefix_length % 8 != 0) {
      ...
      as_v6->sin6_addr.s6_addr[idx] = set
        ? (as_v6->sin6_addr.s6_addr[idx] | prefix)
        : (as_v6->sin6_addr.s6_addr[idx] & ~prefix);
    }
...
inline
void unset_prefix_bits(sockaddr_storage *lower, unsigned int prefix_length) {
  prefix_bit_ops(lower, prefix_length, false);
}

inline
void set_prefix_bits(sockaddr_storage *upper, unsigned int prefix_length) {
